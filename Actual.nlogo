...
to setup-sellers
  ;--------TO CHANGE
  let num-sellers 10  ; Total number of sellers to create
  ...
  sprout-sellers num-sellers[
    ...
    let selling_var ifelse-value (selling_var?) [true] [false]
    ...
    ; set selling? true  ; Initially, all sellers are willing to sell
  ]
end
...
to setup-buyers
  ...
  [
    ...
    let mean-income 5000 ;  NEEDA CHANGE
    ...
    ; Determine if its firsttimer or not
    ...
    ;--------- change the way the offer price is being calcuated --------
...
to buyer-initiate-meeting3
  let lowest-ask-price min [ask-price] of sellers
  let potential-sellers sellers with [ask-price = lowest-ask-price]
  let common-ground-filter potential-sellers with [race = [race] of myself]
  ...
  ifelse any? common-ground-filter [
    ...
    ifelse any? affordable-sellers [
      ...
      print(chosen-seller)
      ask chosen-seller [
        set pcolor yellow   ; Change color of the seller to yellow
        set selling? false  ; Set the selling? variable of the seller to false
      ]
      print (word "Meeting initiated with seller ")
    ] [
  ...
  ask sellers with [selling? = true] [
    ask one-of sellers-here [
      ...
      if selling?[
        ...
        ; Placeholder for ask-price initialization
        set ask-price (random-float 100) + 100
